...
/// |:---:|:---:|:---:|:---:|:---:|
/// | `Compute_calc` | [Compute_calc](struct.Compute_calc.html) | `Input`, `Now`, `Setpoint`, `Kp`, `Ki`, `Kd`, `SampleTime` | `abs_0` | [pid2.lus line 13](../src/lus/pid2.lus.html#13) |
///
/// # Assertions
///
...
#[repr(C)]
  ...
  /// Call to `Compute_calc` ([pid2.lus line 13](../src/lus/pid2.lus.html#13)).
...
/// | Lustre identifier | Struct | Inputs | Outputs | Position |
/// |:---:|:---:|:---:|:---:|:---:|
/// | `limit` | [Limit](struct.Limit.html) | `abs_3` | `abs_4` | [pid2.lus line 39](../src/lus/pid2.lus.html#39) |
/// | `limit` | [Limit](struct.Limit.html) | `abs_1` | `abs_2` | [pid2.lus line 37](../src/lus/pid2.lus.html#37) |
/// | `limit_integral` | [Limit_integral](struct.Limit_integral.html) | `error`, `Ki` | `abs_0` | [pid2.lus line 34](../src/lus/pid2.lus.html#34) |
///
/// # Assertions
///
/// /// No assertions for this system.
...
#[repr(C)]
  ...
  /// Local, alias(Compute_calc.usr.Output): `Compute_calc.res.abs_4`
  pub svar_abs_4: Real,
  /// Local, invisible local: `Compute_calc.res.abs_3`
  pub svar_abs_3: Real,
  /// Local, alias(Compute_calc.impl.usr.outputSum): `Compute_calc.res.abs_2`
  pub svar_abs_2: Real,
  /// Local, invisible local: `Compute_calc.res.abs_1`
  pub svar_abs_1: Real,
  /// Local, alias(Compute_calc.impl.usr.Kitemp): `Compute_calc.res.abs_0`
  pub svar_abs_0: Real,
  /// Local, local: `Compute_calc.impl.usr.Kitemp`
  pub svar_Kitemp: Real,
  ...
  /// Call to `limit_integral` ([pid2.lus line 34](../src/lus/pid2.lus.html#34)).
  pub limit_integral_2: Limit_integral,
  /// Call to `limit` ([pid2.lus line 37](../src/lus/pid2.lus.html#37)).
  pub limit_1: Limit,
  /// Call to `limit` ([pid2.lus line 39](../src/lus/pid2.lus.html#39)).
  pub limit_0: Limit,
}
...
*/   fn init(input: Self::Input) -> Self {
    ...
    let svar_abs_1 = 0f64 ;
    let limit_integral_2 = Limit_integral::init( (
      svar_error,
      svar_Ki,
    ) ) ;
    let (
      svar_abs_0,
    ) = limit_integral_2.output() ;
    ...
    let svar_outputSum = svar_abs_2 ;
    let svar_Kitemp = svar_abs_0 ;
    let svar_abs_3 = (((svar_Kp * svar_error) + (((svar_Kd * svar_dInput) * 1000f64) / svar_SampleTime)) + svar_outputSum) ;
  ...
  fn next(&mut self, input: Self::Input) {
    ...
    /*let limit_integral_2 = */ self.limit_integral_2.next( (
      svar_error,
      svar_Ki,
    ) ) ;
    let (
      svar_abs_0,
    ) = self.limit_integral_2.output() ;
    let svar_Kitemp = svar_abs_0 ;
    let svar_abs_1 = (self.svar_outputSum + (((svar_Kitemp * svar_error) * svar_SampleTime) / 1000f64)) ;
...
/// Stores the state for sub-node `limit_integral`.
///
/// # Inputs
///
/// | Lustre identifier | Type |
...
///
#[repr(C)]
  pub struct Limit_integral {
  /// Input: `limit_integral.usr.error`
  pub svar_error: Real,
  /// Input: `limit_integral.usr.Ki`
  pub svar_Ki: Real,

  /// Output: `limit_integral.usr.y`
  pub svar_y: Real,


}

impl Sys for Limit_integral {
  type Input = (
    Real, // svar_error (limit_integral.usr.error)
    Real, // svar_Ki (limit_integral.usr.Ki)
  ) ;
  type Output = (
    Real, // svar_y (limit_integral.usr.y)
  ) ;
/*  fn arity() -> usize { 2 }
*//*  fn input_of(vec: Vec<String>) -> Result<Self::Input, String> {
        match vec.len() {
...
*/   fn init(input: Self::Input) -> Self {
    // |===| Retrieving inputs.
    let svar_error = input.0 ;
    let svar_Ki = input.1 ;
    
    // |===| Computing initial state.
    let svar_y = ( if (svar_error > 1f64 / 10f64) { 0f64 } else {( if (svar_error < - 1f64 / 10f64) { 0f64 } else {svar_Ki } ) } ) ;
  ...
  pub fn clap_and_run() {
    ...
    if let Some(arg) = args.next() {
      match & arg as & str {
        ...
        "--limit_integral" => super::Limit_integral::run(),
